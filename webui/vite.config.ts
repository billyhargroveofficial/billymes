...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    build: {
      ...
      rollupOptions: {
        output: {
          // History/replay is a sizeable, cohesive static module. Keep it out
          // of the entry asset so recovery work cannot consume the shell's
          // per-file budget; it remains in the same eager cold-start graph.
          manualChunks(id) {
            if (normalizePath(id).endsWith('/src/features/chat/model/chat-history.ts')) {
              return 'chat-history'
            }
          },
        },
